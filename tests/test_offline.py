...
def test_scout_and_decide_attack():
    ...
    from bot.config import MIN_LOOT_TO_ATTACK
    ...
    with patch("bot.battle.read_enemy_loot", return_value=high_loot), \
         patch("bot.battle.deploy_troops") as mock_deploy:
        from bot.battle import scout_and_decide
...
def test_scout_and_decide_skip():
    ...
    with patch("bot.battle.read_enemy_loot", return_value=low_loot), \
         patch("bot.battle.deploy_troops") as mock_deploy, \
         patch("bot.battle.skip_base", return_value=fake_next_img):
        from bot.battle import scout_and_decide
        attacked, next_img = scout_and_decide(img)
...
def test_ensure_on_village_already_there():
    ...
    with patch("bot.main.screenshot", return_value=dummy_img), \
         patch("bot.main.detect_screen_state", return_value="village"), \
         patch("bot.main.tap") as mock_tap:
        from bot.main import ensure_on_village
