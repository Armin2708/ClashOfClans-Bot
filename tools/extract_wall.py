...
def auto_extract(img):
    ...
    """
    os.makedirs("data/templates/walls", exist_ok=True)
    h, w = img.shape[:2]
    ...
    for i, (x, y) in enumerate(gold_positions):
        ...
        if crop.size == 0:
            continue
        path = f"data/templates/walls/wall_gold_{i}.png"
    ...
    for i, (x, y) in enumerate(white_positions):
        ...
        if crop.size == 0:
            continue
        path = f"data/templates/walls/wall_white_{i}.png"
        cv2.imwrite(path, crop)
        print(f"  Saved {path} ({x2-x1}x{y2-y1}) from ({x},{y})")
        saved += 1
...
def manual_extract(img, x, y, size=34):
    ...
    crop = img[y1:y2, x1:x2]
    existing = len([f for f in os.listdir("data/templates/walls") if f.endswith(".png")])
    path = f"data/templates/walls/wall_{existing}.png"
    cv2.imwrite(path, crop)
    print(f"Saved {path} ({x2-x1}x{y2-y1}) from ({x},{y})")
    print("Run: python test.py walls")
